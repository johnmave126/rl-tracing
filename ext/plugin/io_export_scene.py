...
class TracerWritter:
    ...
    def __createTransform(self, mat, el=None):
        ...
        for j in range(4):
            for i in range(4):
                value += str(mat[j][i]) + ","
        transform.appendChild(self.__createElement(
    ...
    def write(self, exportLight, nbSamples):
        ...
        ######################
        if(not exportLight):
            self.scene.appendChild(self.__createElement(
                "integrator", {"type": "av"}))
        else:
            self.scene.appendChild(self.__createElement(
        ...
        sampler.appendChild(self.__createElement(
            "integer", {"name": "sampleCount", "value": str(nbSamples)}))
        ...
        if(len(cameras) == 0):
            ...
            if(len(cameras) > 1):
                self.verbose(
                    "WARN: Does not handle multiple camera, only export the first one")
            self.scene.appendChild(self.write_camera(
                cameras[0]))  # export the first one
        ...
        if(exportLight):
            ...
            for source in sources:
                if(source.data.type == "POINT"):
                    pointLight = self.__createElement(
                        "emitter", {"type": "point"})
                    pos = source.location
                    pointLight.appendChild(self.__createEntry(
                        "point", "position", "%f,%f,%f" % (pos.x, pos.y, pos.z)))
                    self.scene.appendChild(pointLight)
                else:
                    self.verbose(
                        "WARN: Light source type (%s) is not supported" % source.data.type)
    ...
    def write_camera(self, cam):
        ...
        camera.appendChild(self.__createEntry(
            "float", "fov", str(cam.data.angle * 180 / math.pi)))
        camera.appendChild(self.__createEntry(
            "float", "nearClip", str(cam.data.clip_start)))
        camera.appendChild(self.__createEntry(
            "float", "farClip", str(cam.data.clip_end)))
        percent = self.context.scene.render.resolution_percentage / 100.0
        camera.appendChild(self.__createEntry("integer", "width", str(
            int(self.context.scene.render.resolution_x * percent))))
        camera.appendChild(self.__createEntry("integer", "height", str(
            int(self.context.scene.render.resolution_y * percent))))
        trans = self.__createTransform(
            cam.matrix_world, self.__createVector("scale", (1, 1, -1)))
    ...
    def __createMeshEntry(self, filename, matrix):
        ...
        meshElement.appendChild(self.__createElement(
    ...
    def __createBSDFEntry(self, slot):
        ...
        if slot.material.raytrace_mirror.use:
            ...
            bsdfElement.appendChild(self.__createEntry(
                "color", "albedo", "%f,%f,%f" % (c[0], c[1], c[2])))
            return bsdfElement
        ...
        for poly in prevMesh.polygons:
            ...
            if((idMat != -1) and (poly.material_index == idMat)):
                ...
                for vert in vertFaces:
                    ...
                    # Order checking
                    if(not exportNormal):
                        ac = prevMesh.vertices[
                            vert[2]].co - prevMesh.vertices[vert[0]].co
                        ab = prevMesh.vertices[
                            vert[1]].co - prevMesh.vertices[vert[0]].co
                        norm = ab.cross(ac)
                        norm.normalize()
    ...
    def write_mesh_objs(self, mesh):
        ...
        haveMaterial = (len(mesh.material_slots) !=
                        0 and mesh.material_slots[0].name != '')
        ...
        if(not haveMaterial):
            ...
            # add default BSDF
            meshElement = self.__createMeshEntry(
                fileObjPath, mesh.matrix_world)
            bsdfElement = self.__createElement("bsdf", {"type": "diffuse"})
            bsdfElement.appendChild(self.__createEntry(
            ...
            for id_mat in range(len(mesh.material_slots)):
                ...
                fileObjMat = open(self.workingDir +
                                  "/meshes/" + fileObjMatPath, "w")
                shutil.copyfileobj(
                    open(self.workingDir + "/meshes/" + fileObjPath, "r"), fileObjMat)

                # we write all face material specific
                self.write_face(prevMesh, fileObjMat,
                                exportUV, exportNormal, id_mat)

                # We create xml related entry
                meshElement = self.__createMeshEntry(
                    fileObjMatPath, mesh.matrix_world)
